...
const rpcErrors: Record<string, RpcErrorEntry> = {
  ...
  "contract.balance_too_low": {
    message: (errorDetails: any) => {
      return `An operation tried to spend more tokens than the \
${isKTAddress(errorDetails.contract) ? 'contract' : 'account'} has`;
    }
  },
  ...
  "contract.manager.unregistered_delegate": {
    message: (errorDetails: any) => `The delegate ${errorDetails.hash} is unregistered`
  },
  ...
  "gas_exhausted.init_deserialize": {
    message:
      `Gas limit was not high enough to deserialize the transaction parameters or origination \
script code or initial storage, making the operation impossible to parse within the provided gas bounds.`,
  ...
  "implicit.empty_implicit_contract": {
    message: (errorDetails: any) => {
      return isKTAddress(errorDetails.implicit)
        ? "No manager operations are allowed on an empty implicit contract."
        : "The account has not been revealed yet. Please send some XTZ to it and try again.";
    }
  },
  ...
  "michelson_v1.bad_contract_parameter": {
    message:
      `Either no parameter was supplied to a contract with a non-unit parameter type, a non-unit \
parameter was passed to an account, or a parameter was supplied of the wrong type`,
  ...
  "michelson_v1.inconsistent_types": {
    message:
      `This is the basic type clash error, that appears in several places where the equality of \
two types have to be proven, it is always accompanied with another error that provides more context.`
  ...
  "node.state.bad_data_dir": {
    message:
      `The data directory could not be read. This could be because it was generated with an old \
version of the tezos-node program. Deleting and regenerating this directory may fix the problem.`
  ...
  "RPC.Unexpected_error_encoding": {
    message:
      `The RPC returned with an error code, and the associated body was not a valid error \
