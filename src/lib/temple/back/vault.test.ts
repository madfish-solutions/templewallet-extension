...
describe("Vault tests", () => {
  ...
  it("sign tz1 64 bytes test", async () => {
    ...
    const accounts = await vault.fetchAccounts();
    const { publicKeyHash } = accounts[0];
    const signer = await vault.sign(
      publicKeyHash,
      "edskS3DtVSbWbPD1yviMGebjYwWJtruMjDcfAZsH9uba22EzKeYhmQkkraFosFETmEMfFNVcDYQ5QbFerj9ozDKroXZ6mb5oxV"
    );
    expect(signer.sig).toBe(
      "sigo2qRs669qQJnde7ZmdA2EnrS3FVEF3rS13pP1ZC9efizaBfStacNkKSQgoRZAWTEWMHYJ3UmdqqdqKKdLpTqf5D9x14yg"
    );
  });
  ...
  it("sign tz1 32 bytes test", async () => {
    ...
    expect(signer.sig).toBe(
      "sigUYks4nvN3U6CdAXpzAsywHJB3vhBj8VLNsJ3dzT8rkpRQvX18uyiUAwA6T6yue9eK759zxvvEEntWyRukZa8ZM1uZv5NW"
    );
  });
  ...
  it("sign tz2 test", async () => {
    ...
    expect(signer.sig).toBe(
      "sigXqAu36vGDx6fmS9LW8wGSbzAcF2YjW1efYyYfQfNTk8H47HmnvLJtgwgMj5J9fdwirREqJv4NX4JiqcDC2skmBfKx1hCK"
    );
  });
  ...
  it("sign tz2 with bytes producing public key that needs padding test", async () => {
    ...
    expect(signer.sig).toBe(
      "sigVVtP3inCYQYZB1Surjv7wFPukFA62pCtaow3kaYoJQGMMd2fz7P3v7dYAp2eN9FMQb6m6iASUhGfDmuSpQuFshoMEkfpA"
    );
  });
  ...
  it("sign tz3 test", async () => {
    ...
    expect(signer.sig).toBe(
      "sigiqbbZKEuWSB18AHBZw6jYYLfQ7tkRgPynKki7btdnJb844dm4hDgyW8FG2gS8mF41Dqq1wkrQupBuaDQbiTs6G674KWu8"
    );
  });
  ...
  it("sign tz3 encrypted test", async () => {
    ...
    expect(signer.sig).toBe(
      "sigvKqycuSGRBh8vX97MR6Ncu8DqeF36PFUhQo3uPZaGv4MFsPbDTYAzWuUCQCUzttB6f9CZzgddJ5GoQfEWRVbQxE91xwkZ"
    );
  });
  ...
  it("sign tz3 encrypted with bytes producing signature that needs padding test", async () => {
    ...
    expect(signer.sig).toBe(
      "sigZhdgSpmXkRqQ7kbbSPFAn6D2M88VPwa6ijzbjVV8sikMiGhHsGoXbTRhwv7zrvejDq37oSt4BkUm8Bc5TPgkyMPgSM54T"
    );
  });
