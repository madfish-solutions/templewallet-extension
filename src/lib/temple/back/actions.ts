...
export function sendOperations(
  ...
  return withUnlocked(async ({ vault }) => {
    ...
    return new Promise(async (resolve, reject) => {
      if (straightaway) {
        try {
          const { operation, startingBlockHash } = await vault.sendOperations(sourcePkh, network, opParams);

          await safeAddLocalOperation(network.rpcBaseURL, operation);

          resolve({ opHash: operation.hash, startingBlockHash });
        } catch (err: any) {
          reject(err);
        }
      } else {
