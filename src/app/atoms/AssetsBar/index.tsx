...
export const AssetsBar = memo<AssetsBarProps>(({
  ...
  if (searchMode) {
    return (
      <div className={clsx('flex gap-4 items-center px-2 py-3', className)}>
        <div className="flex-1 bg-input-low rounded-lg h-8 flex items-center justify-between px-3 py-0.5">
          <div className="flex gap-0.5 items-center flex-1">
            <IconBase Icon={SearchIcon} size={12} className="text-primary" />
            <input
              type="text"
              value={searchValue}
          ...
          {searchValue && (
            <CleanButton onClick={handleCleanSearch} className="ml-1" />
          )}
        </div>
        <AssetsBarIconButton
          Icon={CloseIcon}
  ...
  if (filtersOpened || manageActive) {
    return (
      <div className={clsx('flex gap-8 items-center px-2 py-3', className)}>
        <SegmentControl
        ...
        <div className="flex gap-2 items-center">
          <AssetsBarIconButton
            Icon={CloseIcon}
            onClick={handleCloseManageOrFilter}
          />
        </div>
      </div>
    );
...
const SegmentControl = memo<SegmentControlProps>(({
  ...
  <div ref={controlRef} className={clsx(styles.controlsContainer, 'flex-1')}>
    <div className={styles.controls}>
      <div
        ref={tokensRef}
        className={clsx(styles.segment, activeTab === 'tokens' ? styles.active : styles.inactive)}
      >
        <input
          type="radio"
          value="tokens"
...
const AssetsBarIconButton = memo<AssetsBarIconButtonProps>(({ Icon, onClick, active }) => (
  <Button
    className={clsx(
      ...
      active
        ? 'bg-primary-low text-primary'
        : 'bg-white text-primary border-0.5 border-lines hover:bg-grey-4'
    )}
    onClick={onClick}
  >
