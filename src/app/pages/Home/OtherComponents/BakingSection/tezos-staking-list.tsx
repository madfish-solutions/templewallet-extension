...
export const TezosStakingList = memo<Props>(({ network, accountPkh, bakerPkh, cannotDelegate }) => {
  ...
  const { data: requests } = useUnstakeRequests(rpcBaseURL, accountPkh, true);
          ...
          <>
            {staked && (
              <StyledButton
                ...
                onClick={() => navigate(`/staking/${chainId}`)}
              >
                <T id="unstake" />
              </StyledButton>
            )}
...
const UnstakeRequestItem = memo<UnstakeRequestItemProps>(
  ({ delegate, cycle, amount, index, cyclesInfo, blockLevelInfo, gasTokenSymbol, blockExplorerUrl }) => {
      ...
      <StakingCard
        topInfo={
          <>
            ...
            {delegate ? (
              <StyledButton color="primary" size="S">
                <T id="finalize" />
              </StyledButton>
            ) : (
