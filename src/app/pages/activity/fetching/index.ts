...
import {
	detectTokenStandard,
} from 'lib/temple/assets/tokenStandard';
...
export default async function fetchActivities(
   ...
   const {
	...
	const groups = await _fetchOperGroupsForOperations(
		chainId,
		operations,
		olderThan,
	);
   ...
   const {
	...
	if(assetSlug) {
		...
		if(assetSlug === TEZ_TOKEN_SLUG) {
			return await fetchOperations_TEZ(
				chainId,
				accAddress,
				pseudoLimit,
				olderThan,
			...
			return await fetchOperations_Contract(
				chainId,
				accAddress,
				pseudoLimit,
				olderThan,
	...
	return await fetchOperations_Any(
		chainId,
		accAddress,
		pseudoLimit,
		olderThan,
