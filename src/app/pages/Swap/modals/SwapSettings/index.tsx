...
const SwapSettingsModal = memo<SelectTokenModalProps>(
  ({ currentSlippageTolerance, opened, onRequestClose, onSubmit }) => {
    const customLabel = t('custom');
    const options: (number | typeof customLabel)[] = [0.5, 1, customLabel];

    const [appliedSlippage, setAppliedSlippage] = useState<number | typeof customLabel>(currentSlippageTolerance);
