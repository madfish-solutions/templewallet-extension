...
const ExchangeStep: FC<Props> = ({exchangeData, setExchangeData, setStep, step, isError, setIsError}) => {
    ...
    useEffect(() => {
        if (exchangeData!.status === "success" || exchangeData!.status === "exchanging") {
    ...
    return (
        <>
            {(exchangeData!.status === 'exchanging' || exchangeData!.status === 'confirmation' || exchangeData!.status === 'overdue') && (
                <>
                    <div className='m-auto'>
                        <p className='text-center text-base mt-4 text-gray-700'>
                            {(exchangeData!.status  === 'confirmation' || (exchangeData!.status === 'overdue' && step === 2)) && <T id={'confirmation'}/>}
                            {(exchangeData!.status === 'exchanging' || (exchangeData!.status === 'overdue' && step === 3)) && <T id={'exchanging'}/> }
                        </p>
                        <p className='text-center text-xs text-gray-700 mt-1'><T id={'waitMessage'}/></p>
                    </div>
                    {isError ? (
                        <ErrorComponent exchangeData={exchangeData} setExchangeData={setExchangeData} setStep={setStep} setIsError={setIsError} />
                    ) : (
                        <>
                            <Divider style={{marginTop: '2.25rem'}}/>
                            <div className='flex justify-between items-baseline mt-4'>
                                <p className='text-gray-600 text-xs'><T id={'transactionId'}/></p>
                                <span>
                            ...
                            <div className='flex justify-between items-baseline mt-4'>
                                <p className='text-gray-600 text-xs'><T id={'youSend'}/></p>
                                <p style={{color: '#1B262C'}} className='text-xs'>{exchangeData!.amount_from} {exchangeData!.coin_from}</p>
                            </div>
                            <div className='flex justify-between items-baseline mt-2'>
                                <p className='text-gray-600 text-xs'><T id={'youReceive'}/></p>
                                <p style={{color: '#1B262C'}} className='text-xs'>{exchangeData!.amount_to} {exchangeData!.coin_to}</p>
                            </div>
                            <div className='flex justify-between items-baseline mt-4'>
                                <p className='text-gray-600 text-xs'>
                                    <T id={'depositAddressText'} substitutions={[
                                        exchangeData!.coin_from
                                    ]} />
                                </p>
                            ...
                            <div className='flex justify-between items-baseline mt-4'>
                                <p className='text-gray-600 text-xs'><T id={'recipientAddress'} /></p>
                                <p style={{color: '#1B262C'}} className='text-xs'>
            ...
            {exchangeData!.status === 'success' && (
                <>
                    <div className='m-auto'>
                        <p className='text-center text-base mt-4 text-gray-700'><T id={'completed'} /></p>
                    ...
                    <div className='flex justify-between items-baseline mt-4'>
                        <p className='text-gray-600 text-xs'><T id={'sendTime'} /></p>
                        <p style={{color: '#1B262C'}} className='text-xs'>{sendTime.toLocaleDateString(getCurrentLocale(), dateFormatOptions)}</p>
                    </div>
                    <div className='flex justify-between items-baseline mt-4'>
                        <p className='text-gray-600 text-xs'><T id={'youSend'} /></p>
                        <p style={{color: '#1B262C'}} className='text-xs'>{exchangeData!.amount_from} {exchangeData!.coin_from}</p>
                    ...
                    {(exchangeData!.hash_out && exchangeData!.hash_out_link) && (
                        <div className='flex justify-between items-baseline mt-2'>
                            <p className='text-gray-600 text-xs'><T id={'inputHash'} /></p>
                            <p style={{color: '#1B262C'}} className='text-xs'>
                    ...
                    <div className='flex justify-between items-baseline mt-4'>
                        <p className='text-gray-600 text-xs'><T id={'recipientXtzAddress'} /></p>
                        <p style={{color: '#1B262C'}} className='text-xs'>
                            <HashShortView hash={exchangeData!.destination_address} />
                        </p>
                    </div>
