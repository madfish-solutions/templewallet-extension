...
const DEXTER_INITIAL_TOKENS = new Map<string, string[]>([
  [
...
export const getAssetExchangeData = (
  ...
  const [address, tokenIdStr] = assetSlug.split("_");
  return tokensExchangeData[address]?.[+tokenIdStr]?.[exchanger];
};
...
export const [SwappableAssetsProvider, useSwappableAssets] = constate(
  ({ initialAssetKey }: SwappableAssetsProviderProps) => {
    ...
    const [qsStoredTokenSlugs, setQsStoredTokenSlugs] = useStorage<string[]>(
