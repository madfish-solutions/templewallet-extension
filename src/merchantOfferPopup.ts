import interMedium from 'inter-ui/Inter (web)/Inter-Medium.woff2';
import interRegular from 'inter-ui/Inter (web)/Inter-Regular.woff2';
import interSemiBold from 'inter-ui/Inter (web)/Inter-SemiBold.woff2';
...
function injectPopup(offer: MerchantOffer, domain: string) {
  ...
  // @font-face must be in the document scope — shadow DOM doesn't support it
  const fontStyle = document.createElement('style');
  fontStyle.textContent = getFontFaceRules();
  ...
  function render() {
    ...
    if (settingsOpen) {
      ...
      snoozeBtn.innerHTML = `<svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      disableBtn.innerHTML = `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      setTimeout(() => shadow.addEventListener('click', onOutsideClick, { once: true }));
    }
...
function getFontFaceRules() {
  const fontUrl = (asset: string) => browser.runtime.getURL(asset);
  ...
  return `
    @font-face {
      ...
      font-weight: 400;
      font-display: swap;
      src: url("${fontUrl(interRegular)}") format("woff2");
    }
    ...
    @font-face {
      ...
      font-weight: 500;
      font-display: swap;
      src: url("${fontUrl(interMedium)}") format("woff2");
    }
    ...
    @font-face {
      ...
      font-weight: 600;
      font-display: swap;
      src: url("${fontUrl(interSemiBold)}") format("woff2");
    }
  `;
}
