...
const defaultMessages = JSON.parse(fs.readFileSync(path.join(LOCALES_PATH, DEFAULT_LOCALE, MESSAGES_FILE), 'utf8'));

const fileToImport = getArgValue('--file');
const locale = getArgValue('--locale');

const importMessages = JSON.parse(fs.readFileSync(path.join(process.cwd(), fileToImport), 'utf8'));
...
fs.writeFileSync(path.join(localeDir, MESSAGES_FILE), JSON.stringify(toImport, null, 2), 'utf8');
